"""
run_ttm_grid_experiment.py — Batch-run the full TTM FT-Head grid and export one Excel workbook.
...
def main():
    parser = argparse.ArgumentParser(description="Run the full TTM FT-Head experiment grid")
    ...
    output_excel = Path(args.output_excel) if args.output_excel else (
        VOLARE_RESULTS_DIR / "fine_tune_head" / "metrics" / "ttm_full_grid_results.xlsx"
    )
    ...
    run_id = 0
    for context_length, horizon, selection_metric, train_target_transform in product(
        args.contexts,
        args.horizons,
