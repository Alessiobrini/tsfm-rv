"""
run_moirai_moe_grid_experiment.py — Batch-run the full Moirai-MoE FT-Head grid and export one Excel workbook.
...
def main():
    parser = argparse.ArgumentParser(description="Run the full Moirai-MoE FT-Head experiment grid")
    ...
    output_excel = Path(args.output_excel) if args.output_excel else (
        VOLARE_RESULTS_DIR / "fine_tune_head" / "metrics" / "moirai_moe_full_grid_results.xlsx"
    )
    ...
    for context_length, horizon in product(args.contexts, args.horizons):
        _prewarm_moirai_moe_cache(tickers, context_length, horizon, runtime)
        ...
        for selection_metric, train_target_transform in product(
            ...
            run_start = time.time()
            print(
                f"[{len(completed_keys) + 1}/{total_runs}] Running Moirai-MoE "
