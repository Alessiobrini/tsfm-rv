"""
run_chronos_grid_experiment.py — Batch-run the full Chronos-Bolt-S FT-Head grid and export one Excel workbook.
...
def _prewarm_chronos_cache(
    tickers: list[str],
    context_length: int,
    horizon: int,
    runtime: FineTuneRuntimeConfig,
) -> None:
    """Populate the Chronos raw-output cache once per (ctx, h) over the full panel."""
    ...
    for ticker in tickers:
        ...
        adapter._raw_outputs(np.asarray(contexts, dtype=np.float32))


def main():
    parser = argparse.ArgumentParser(description="Run the full Chronos-Bolt-S FT-Head experiment grid")
    ...
    output_excel = Path(args.output_excel) if args.output_excel else (
        VOLARE_RESULTS_DIR / "fine_tune_head" / "metrics" / "chronos_full_grid_results.xlsx"
    )
    ...
    for context_length, horizon in product(args.contexts, args.horizons):
        _prewarm_chronos_cache(tickers, context_length, horizon, runtime)
