...
- B6 table_context_sensitivity — **DEFERRED (cluster)**. Stale table replaced with honest placeholder; paragraph softened + red pending note. Aggregator written (`gen_context_sensitivity.py`, refuses partial sweeps); SLURM `run_rev_context_sensitivity.slurm` ready (128/256/512; 1000 from main).
...
- B9 averaged-target appendix — **DEFERRED (cluster)**. results/volare_avg absent locally AND on DCC, so the asserted "ranking reproduced" claims were UNVERIFIED -> removed; paragraph softened + red pending note. SLURM `run_rev_avg_target.slurm` created (baselines+TSFM, --target-kind avg).
